...
Read_Write_Relative Relative_Sector_Buffer <>	;				;AN000;
	...
	IF GSOCOPYRIGHT
BiosFile db	"x:\GSOBIO.COM", 0
DosFile db	"x:\GSODOS.COM", 0
	ELSE
BiosFile db	"x:\GSOIO.SYS", 0
DosFile db	"x:\GSODOS.SYS", 0
	ENDIF
...
Goto_EndSwitchCheck:
	jmp	EndSwitchCheck
						;There is a problem with /N/T in that GSOBIO will default to a BPB with the
...
;	      using only switchs. All defined DOS GSO0 sizes are hardcoded,
;	      in case a drive type of other is encountered that doesn't
;	      qualify as a DOS GSO0 defined drive. Exit with error message if
...
data	segment public	para	'DATA'
	...
	if GSOCOPYRIGHT
biosFilename db "x:\GSObio.com",0
dosFilename db	"x:\GSOdos.com",0
	else
biosFilename db "x:\GSOIO.SYS",0
dosFilename db	"x:\GSODOS.sys",0
	endif
...
; fix attribute of GSObio and GSOdos
	lea	bx,scratchBuffer
...
; structure of the GSO hard disk boot sector:
GSOBoot STRUC
