...
IF GSOCOPYRIGHT
BIOSName		DB   "A:\GSOBIO.COM",0 ; z string for target name
DOSName 		DB   "A:\GSODOS.COM",0 ; z string for target name
ELSE
BIOSName		DB   "A:\GSOIO.SYS",0
DOSName 		DB   "A:\GSODOS.SYS",0
ENDIF
...
SourceSpec		DB   "A:"
			DB   53 dup (0)
IF GSOCOPYRIGHT
SourceBIOS		DB   "\GSOBIO.COM",0
ELSE
SourceBIOS		DB   "\GSOIO.SYS",0
ENDIF
...
SourceDOSName		DB   "A:"
			DB   53 dup (0)
IF GSOCOPYRIGHT
SourceDOS		DB   "\GSODOS.COM",0
ELSE
SourceDOS		DB   "\GSODOS.SYS",0
ENDIF
...
GSOBIO_LOW		DW   0		;length of GSOBIO on disk
GSOBIO_HIGH		DW   0
GSODOS_LOW		DW   0		;length of old GSODOS on disk
GSODOS_HIGH		DW   0
...
IF GSOCOPYRIGHT
FCBDOS			DB   "GSODOS  COM"
FCBBIO			DB   "GSOBIO  COM"
ELSE
FCBDOS			DB   "GSODOS   SYS"
FCBBIO			DB   "IO      SYS"
ENDIF
...
;	by DOS is used as an input buffer for transfering GSOBIO and GSODOS.
;
...
;	This is required in case output is directed to NULL. (See DOS GSO0
;	PTR P71)
;
...
;		endif
;	endif
;	remove	blanks in \GSOBIO.COM
;	remove	blanks in \GSODOS.COM
;	ret
;
...
;  $if	e				; if a source was specified	       ;AN021;
   JNE $$IF42
       lea  si,SourceSpec		; copy source for GSODOS.COM	       ;AN021;
...
;  $if	nc				; if no error to this point	       ;AN021;
   JC $$IF48
       ...
       lea  di,SourceBiosName		; move GSOBIO.COM into place	       ;AN021;
       ...
       lea  di,SourceDOSName		; move GSODOS.COM into place	       ;AN021;
       add  di,SourceSize		; move to end of specified part        ;AN021;
...
;*******************************************************************************
;
;Description:	    Ensure that the the files GSOBIO and GSODOS are available
...
;
;Input: 	    GSOBIO and GSODOS on source media
;
;Output: no error - CF = 0
;	    error - CF = 1	  AX = return code (message #)
;
...
$$DO62:
       ...
       mov  dx,OFFSET SourceBIOSName	; look on source for GSOBIOS
...
;      $if  nc,and			; if no error and....................  ;AC000;
       ...
       mov  dx,OFFSET SourceDOSName	; look on source for GSODOS
       mov  CX,DOS_system_atrib 	; its an 'everything' file
       mov  ah,Find_First		; do a find first INT21
       INT  21h 			;      Find_First <4Exx>
...
;      $if  e				; if correct version of GSOBIO	       ;AN019;
       JNE $$IF63
...
;*******************************************************************************
;
;Description:  Read in as much of GSOBIOS and GSODOS as room permits
;
;Called Procedures: None
;
;Input: 	    None
...
;  $if	b				; if amount to read < buffer..:        ;AC000;
   JNB $$IF90
       ...
       mov  cx,BIOSLenLow		; set length to GSOBIO length
...
;      $if  b				; if its less .................:       ;AC000;
	   ...
	   mov	cx,DOSLenLow		; set length to GSODOS length
...
;	if no error and
;		check that GSOBIO,GSODOS are in right place (CALL Verify_File_Location)
;	if no error and
...
;  $if	nc,and				; no error and			       ;AN000;
   ...
   call Verify_File_Location		; check that GSOBIO,GSODOS are in right;AN000;
					;    place
...
;      $if  c				; error - check if old version dest    ;AN000;
	   ...
	   cmp	al,old_type_media	; is it GSO but < GSO ? 	       ;AN000;
...
;
;Description: Determines if GSOBIO and GSODOS are the first two directory
;	      entries, or if these entries are empty. If so, find the size
;	      of the files if they exist. If spaces not empty and don't
;	      contain GSOBIO and GSODOS, set fail flag and load return code.
;	      Also determines if existing GSOBIO starts in cluster 2. If not
...
;			reset fail flag
;		else
;			if first file GSOBIO.COM
;				get GSOBIO Size
;				indicate we found GSOBIO
;				clear error flag
;			else
;				call Move_DIR_Entry
;				call Free_Cluster
...
;				reset fail flag
;			else
;				if second file GSODOS.COM
;					get GSODOS size
;					indicate we found GSODOS
;					reset fail flag
;				else
;					call Move_DIR_Entry
;				endif
...
;      $if  e				; first file GSOBIO.COM 	       ;AC000;
       JNE $$IF120

	   dec	BIOSEntFree		; indicate we found GSOBIO ( = 0)
	   mov	si,bp
	   mov	ax,word ptr ds:[si].dir_size_l ; Get the size of GSOBIO        ;AC000;
	   mov	word ptr GSOBIO_Low,ax
	   mov	ax,word ptr ds:[si].dir_size_h ;			       ;AC000;
	   mov	word ptr GSOBIO_High,ax
	   cmp	ds:[si].dir_first,2	; does GSOBIO own Clust 2?	       ;AC005;
;	   $if	e			; if so 			       ;AC005;
	   JNE $$IF121
...
$$IF131:
	       ...
	       mov  di,OFFSET FCBDOS	;   see if it is GSODOS
	       mov  cx,file_spec_length ; length of name
	       rep  cmpsb		; compare it
	       mov  si,bp		; restore pointer to start	       ;AC000;

;	       $if  e			; if second file is GSODOS.COM.        ;AC000;
	       JNE $$IF133

		   dec	DOSEntFree	; indicate we found GSODOS
		   mov	ax,word ptr ds:[si].dir_size_l ; Get the size of       ;AC000;
		   mov	word ptr GSODOS_Low,ax ;	   file for GSODOS
		   mov	ax,word ptr ds:[si].dir_size_h ;		       ;AC000;
		   mov	word ptr GSODOS_High,ax
		   clc			; reset fail flag		       ;AN000;
...
;	      space and the space taken up by GSOBIO and GSODOS to install the
;	      new GSOBIO and GSODOS. Routine will set fail flag and load return
;	      code if there is not enough room.
;
;	      Here we make some VERY IMPORTANT assumptions.
;
;	      1) If GSOBIO exists on the disk currently, we assume it is in the
;		 correct place, i.e. at the front of the data area & contiguous.
;	      2) The stub loader portion of GSOBIO is less than 2048 bytes long.
;		 This number comes about by assuming we will never overlay
;		 anything smaller than 1920 bytes (DOS 1.10 GSOBIO size). This
;		 can be expanded to 2048 if we also assume the smallest possible
;		 cluster length is 512 bytes.
;
;	      Therefore, if we have an empty disk or GSOBIO exists, then we have
;	      enough contiguous room to install the portion of GSOBIO that
...
;	convert current GSOBIO into cluster size (CALL Get_Cluster)
;	convert current GSODOS into cluster size (CALL Get_Cluster)
;	get total number of clusters available
;	convert new GSOBIO into cluster size (CALL Get_Cluster)
;	convert new GSODOS into cluster size (CALL Get_Cluster)
...
;******************-  END  OF PSEUDOCODE -**************************************
   ...
   mov	ax,GSOBIO_Low			; low result in AX, High result in DX
   mov	dx,GSOBIO_High
   call Get_Cluster			; convert old GSOBIO into cluster size

   add	Number_Free_Clusters,ax 	; add it to available space

   mov	ax,GSODOS_Low			; low result in AX, High result in DX
   mov	dx,GSODOS_High
   call Get_Cluster			; convert old GSODOS into cluster size
   ...
   call Get_Cluster			; convert new GSOBIO into cluster size
   ...
   call Get_Cluster			; convert new GSODOS into cluster size
...
;	START  Do_SYS
;
;	create new GSOBIO and GSODOS, in place if exist (CALL CREATE_SYSTEM)
;	if no error
;		search
...
;******************-  END  OF PSEUDOCODE -**************************************
   ...
   call CREATE_SYSTEM			; create GSOBIO and GSODOS, in place   ;AN000;
...
;	START  Create_System
;
;	create GSOBIO (CALL Create_File)
;	if no error and
;		save handle to GSOBIO_Handle
;		create GSODOS (CALL Create_File)
;	if no error
;		save handle to GSODOS_Handle
;		reset fail flag
;	endif
;	ret
;
...
;******************-  END  OF PSEUDOCODE -**************************************
   ...
   mov	[open_seg],ds
   mov	dx,OFFSET BIOSName		; point to GSOBIO ASCIIZ string
   mov	al,[BIOSEntFree]		; get status of GSOBIO		       ;AN006;
   mov	[EntryFree],al			; update file status (0 = found,1 = not;AN006;

   call Create_File			; create GSOBIO 		       ;AN000;

;  $if	nc,and				; no error and			       ;AC000;
   JC $$IF154

   mov	BIOSOutFH,ax			; save handle to GSOBIO_Handle
   mov	dx,OFFSET DOSName		; pointer to GSODOS ASCIIZ string
   mov	al,[DOSEntFree] 		; get status of GSODOS		       ;AN006;
   mov	[EntryFree],al			; update file status (0 = found,1 = not;AN006;

   call Create_File			; create GSODOS 		       ;AN000;

;  $if	nc				; no error			       ;AC000;
   JC $$IF154

       mov  DOSOutFH,ax 		; save handle to GSODOS_Handle
...
;Last Update: 9/23/87
;
;Description: Remove the read only attribute from GSOBIO and GSODOS. If
...
;      $if  e				;   GSOBIO was not there	       ;AC000;
       JNE $$IF160
...
;*******************************************************************************
;
;Description:	    Write out as much of GSOBIOS and GSODOS as is in memory.
;
;Called Procedures: None
;
;Input: 	    None
...
;	subtract the start of GSODOS
;	if lenght is non-zero and . . . . . . . . . . . . . . . .
;		load GSOBIOS handle				:
;		write out GSOBIOS (INT21 Write + 00 <4000>)	:
;	if no error and . . . . . . . . . . . . . . . . . . . . :
...
;		if lenght is non-zero and
;			load GSODOS handle
;			write out GSODOS (INT21 Write + 00 <4000>)
;		if no error and
;		if not all data written . . . . . . . . . . . . . . . . :
;			set fail flag
;		endif
...
;  $if	nz,and				; if lenght is non-zero and.......     ;AC000;
   ...
   mov	bx,BIOSOutFH			; load GSOBIOS handle		  :
   mov	ah,Write			; write out GSOBIOS		  :
...
;      $if  nz,and			; if lenght is non-zero and........    ;AC000;
       ...
       mov  bx,DOSOutFH 		; load GSODOS handle		  :
       mov  ah,Write			; write out GSODOS		  :
       INT  21h 			; Write + 00 <4000>		  :
...
;*******************************************************************************
;
;Description:	    Finish off with GSOBIOS and GSODOS
;
...
;	START  Do_End
;
;	finish off and close GSOBIOS and GSODOS (CALL Close_Files)
...
;******************-  END  OF PSEUDOCODE -**************************************
   ...
   call Close_File			; finish off & close GSOBIOS and GSODOS;AN000;
...
;*******************************************************************************
;
;Description:	    Set date and time on GSOBIOS and GSODOS and close
;		    them.
;
;Called Procedures: None
;
...
;
;Output:	    GSOBIOS and GSODOS closed
;
;Change History:    Created	   5/01/87	   FG
;
;******************* END OF SPECIFICATIONS *************************************
...
;******************-  END  OF PSEUDOCODE -**************************************
   ...
   mov	cx,BIOSTime			; load GSOBIOS time and date
   mov	dx,BIOSTime+2
   mov	bx,BIOSOutFH			; load GSOBIOS handle
   mov	ax,(File_Times SHL 8) + set	; update file times
   INT	21h				; File_Times + 01 <5701>
   mov	ah,Close			; close file GSOBIO
   INT	21h				; Close + not_used <3Exx>
   mov	cx,DOSTime			; load GSODOS time and date
   mov	dx,DOSTime+2
   mov	bx,DOSOutFH			; load GSODOS handle
   mov	ax,(File_Times SHL 8) + set	; update file times
   INT	21h				; File_Times + 01 <5701>
   mov	ah,Close			; close file GSODOS
   INT	21h				; Close + not_used <3Exx>
...
;  $if	ne				; only do a GET if DOS GSO0		;AN019;
   JE $$IF177
       lea  dx,IOCtl_Buf		; point to output buffer	       ;AN000;
...
;  $if	c				; if it is pre GSO0 GSO format	       ;AN000;
   JNC $$IF185
...
$$IF196:
       ...
       cmp  DOS_VER,0			;				       ;AN019
;      $if  e				; only do a SET if DOS GSO0
       JNE $$IF198
	   mov	bl,TargDrvNum		; Drive number			       ;AN000;
	   lea	dx,IOCtl_Buf		; point to output buffer	       ;AN000;
...
;			  pre DOS GSO0 media (ie: it MUST be a 12 or old style
;			  16 bit FAT
;
