...
; Hooks into existing routines in GSOBIO block driver via Int 2F mpx # 8.
...
;==============================================================================
;REVISION HISTORY:
;AN000 - New for DOS Version GSO0 - J.K.
;AC000 - Changed for DOS Version GSO0 - J.K.
;AN00x - PTM number for DOS Version GSO0 - J.K.
;==============================================================================
...
not_init:
; Because we are passing the call onto the block driver in GSOBIO, we need to
...
; opposed to the one that is relevant to this device driver.
	...
	int	2fH
;
; We need to preserve the flags that are returned by GSOBIO. YUK!!!!!
;
	pushf
	pop	bx
	add	sp,2
...
ERR$EXIT:
	...
	mov	word ptr ds:[bx.CONFIG_ERRMSG], -1 ;AN009;Show GSOBIO error message too.
	JMP	SHORT ERR1
