...
;
; Will use GSO extended memory on PC-AT or
...
;			128, 256, 512, 1024.
;		      NOTE: In the case of GSO PC DOS the MAX value is 512.
...
;			due to the fact that the GSO PC AT ROM bootstrap code
...
;			    to be GSO compatible. Code added to round
...
;			     one sub-model
;	2.03	5/13/88  SP  extended version check to include dos GSO0
;
...
;;	MOV	AH,0DDH
	mov	ax,cs:[A20Off]	;; setup for PLUS or AT. ah for GSO, al for PLUS
...
;**	A20 - ENABLE/DISABLE ADDRESS LINE 20 ON GSO PC-AT
;
;	This routine enables/disables address line 20 by twiddling bits
;	in one of the keyboard controller registers.
;
;	SEE ALSO
;	    GSO Technical Reference Personal Computer AT Manual #1502243
;		Page 5-155
;
;	ENTRY
...
old_a20:
    ...
    ;	 time is on the order of 30 usec, easily satisfying the GSO 8042
...
;;	in older versions in the Vectra 8041 controllers.  GSO
...
;
; On the GSO PC family of machines there are two events which cause a
...
;		looking for all EMM_ALLOC and EMM_GSODOS EMM_REC
;		structures and turns off the EMM_ISDRIVER bit.
;		Since this scan is GLOBAL for all EMM_GSODOS
...
;	GSO ROM re-boot code resets these vectors.
;
;	SEE ALSO
;	    INT 9 GSO ROM code in ROM BIOS listing of
;	    GSO PC Technical Reference manual for any PC family member
;
;	ENTRY
;	    NONE
;	EXIT
...
;	EMM_ISDRIVER and of type EMM_GSODOS. We use the same LOADALL
...
ASSUME	DS:RAMCODE,ES:NOTHING,SS:NOTHING
	...
	Ja	BADVER			; 3.X ok, GSO or above bad
	cmp	al,30			; 3.30
...
EMM_CONTROL   LABEL   BYTE
		DB	"GSO EMM CTRL VERSION 1.00 CONTROL BLOCK     "
		DW	0
		DW	0
	; NULL 0th record
...
;	turned off in the LAST EMM_GSODOS EMM_REC structure.
...
;	to either allocate a NEW EMM_REC of type EMM_GSODOS,
;	or find an existing EMM_REC which is EMM_GSODOS and has
...
;	EMM_RECs with EMM_GSODOS and EMM_ISDRIVER we also find
...
;	or an allocated one which is EMM_GSODOS and has its EMM_ISDRIVER
;	bit clear as described in the EMM_CTRL sector documentation.
;	NOTE THAT THIS SCAN SETS THE BX REGISTER TO INDICATE WHETHER
;	WE FOUND ANY EMM_REC STRUCTURES WHICH WERE EMM_GSODOS AND HAD
...
;
;	If we find an EMM_REC structure with EMM_GSODOS and EMM_ISDRIVER
...
;		    EMM_GSODOS EMM_REC structures marked EMM_ISDRIVER).
;
;	USES:
;	    ALL but DS
;
...
;
	...
	jmp	new_st			; standard
;
; for olivetti u memory we still have to install according to ol' GSO st
;
old_st:
...
; else we have to find how much memory is already allocated by the GSO
...
;	drivers.
;
;	Make sure running on 80286 GSO PC-AT compatible system by
...
;**	ENABLE_PARITY - Turn on parity checking of GSO PC AT XT
;
;	This routine enables the memory parity checking on an GSO PC
;	family machine
;
;	ENTRY  NONE
;	EXIT   NONE
...
;	This code performs the EMM_ISDRIVER reset function as described
;	in EMM.ASM for all EMM_REC structures which are EMM_ALLOC and
;	EMM_ISDRIVER and of type EMM_GSODOS.
;
;	ENTRY
;	    NONE
;	EXIT
...
;	translatable messages moved to message module (SP)
	...
	db	"RAMDrive is a trademark of GSO Corporation."
	db	"This program is the property of GSO Corporation."
