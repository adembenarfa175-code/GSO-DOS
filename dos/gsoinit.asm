...
; AN007 PTM 3957 - fake version for GSOCACHE.COM
...
include fastopen.inc
	...
	I_need	IFS_DOS_CALL,DWORD	 ; IFS GSODOS CALL entry      ;AN000;
...
ASSUME	SS:DOSGROUP
	...
	MOV	DS:[addr_int_GSO],AX
	MOV	AX,CS
	MOV	DS:[addr_int_GSO+2],AX
	POP	DS
...
;;;;;;	MOV	[GSODOS_SIZE],BP ;MS. save it for information
	MOV	DX,CS
...
;F.C Modification start  DOS 3.3
	...
	MOV	WORD PTR ES:[BUFFHEAD+2],ES	  ;LB. DOS GSO0 buffer head pointer ;AN000;
	...
	MOV	ES:[FETCHI_TAG],22642	  ; TAG for GSO,
...
;The following entries don't expect version GSO
...
Version_Fake_Table:			    ;AN007  starting address for special
	DB	12,"GSOCACHE.COM",3,40,255  ;AN007  GSOcache     1
	DB	12,"GSOCACHE.SYS",3,40,255  ;AN007  GSOcache     2
